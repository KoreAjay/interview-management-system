        ...
        <header class="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 lg:px-8 py-4 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-400">Post-interview decisions and feedback</p>
                </div>
            </div>
        </header>
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <form action="{{ url()->current() }}" method="GET" class="flex items-center gap-3">
                        <div class="relative">
                            <select name="status" onchange="this.form.submit()" 
                                class="appearance-none bg-white border border-slate-200 rounded-2xl px-5 py-2.5 pr-10 text-xs font-bold text-slate-600 focus:outline-none focus:border-emerald-500 transition-all cursor-pointer">
                        ...
                        @if(request('status'))
                            <a href="{{ url()->current() }}" class="text-xs font-bold text-rose-500 hover:text-rose-600 px-2 transition-colors">
                                Reset Filter
                            </a>
                        @endif
                    </form>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button class="p-2 bg-white border border-slate-200 rounded-xl text-slate-600 hover:bg-slate-50 transition-colors shadow-sm"><i class="bi bi-file-earmark-pdf"></i></button>
                        <button class="p-2 bg-white border border-slate-200 rounded-xl text-slate-600 hover:bg-slate-50 transition-colors shadow-sm"><i class="bi bi-file-earmark-excel"></i></button>
                    </div>
                </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-50/50 border-b border-slate-100">
                                    ...
                                    <th class="px-8 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Feedback</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-50">
                                @forelse($interviews as $interview)
                                    <tr class="table-row-hover transition-all duration-200 group">
                                        <td class="px-8 py-5">
                                            <div>
                                                <p class="text-sm font-bold text-slate-800 group-hover:text-emerald-600 transition-colors">{{ $interview->candidate->name }}</p>
                                                <p class="text-[11px] text-slate-400">{{ $interview->candidate->email }}</p>
                                            </div>
                                        </td>
                                        <td class="px-6 py-5 text-center">
                                            @php $isSelected = $interview->candidate->status === 'selected'; @endphp
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-tight {{ $isSelected ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600' }}">
                                                <i class="bi {{ $isSelected ? 'bi-check-circle-fill' : 'bi-x-circle-fill' }} mr-1.5"></i>
                                                {{ $isSelected ? 'Selected' : 'Rejected' }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-5">
                                            <div class="flex items-center gap-2">
                                                <div class="w-7 h-7 bg-slate-100 rounded-lg flex items-center justify-center text-[10px] font-bold text-slate-500 border border-slate-200">
                                                    {{ strtoupper(substr($interview->interviewer->name ?? '?', 0, 1)) }}
                                                </div>
                                                <span class="text-xs font-semibold text-slate-600">{{ $interview->interviewer->name ?? 'Unassigned' }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-5 text-center">
                                            @php $rating = $interview->feedback->rating ?? 0; @endphp
                                            <div class="inline-flex items-center gap-1 bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
                                                <span class="text-sm font-black {{ $rating >= 4 ? 'text-emerald-500' : ($rating >= 3 ? 'text-amber-500' : 'text-slate-400') }}">
                                                    {{ $rating ?: '-' }}
                                                </span>
                                                <i class="bi bi-star-fill text-[10px] {{ $rating > 0 ? 'text-amber-400' : 'text-slate-200' }}"></i>
                                            </div>
                                        </td>
                                        <td class="px-8 py-5">
                                            <div class="flex items-center justify-between gap-4">
                                                <p class="text-xs text-slate-500 line-clamp-1 max-w-xs italic">
                                                    "{{ $interview->feedback->remarks ?? 'No feedback recorded' }}"
                                                </p>
                                                <a href="#" class="opacity-0 group-hover:opacity-100 transition-opacity p-1 text-emerald-600">
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-8 py-20 text-center">
                                            <div class="flex flex-col items-center">
                                                ...
                                                <h4 class="text-slate-800 font-bold">No matches found</h4>
                                                <p class="text-slate-400 text-xs mt-1">Try adjusting your filters or search criteria.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
